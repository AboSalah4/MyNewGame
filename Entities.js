...
const Box = props => {
  ...
  return (
    <View style={{
      ...
      backgroundColor: props.color, borderRadius: 5
    }} />
  );
};
...
const Circle = props => {
  ...
  return (
    <View style={{
      ...
      borderRadius: radius, backgroundColor: props.color
    }} />
  );
};
...
export default (hardMode) => {
  ...
  let Floor = Matter.Bodies.rectangle(200, 600, 400, 20, { isStatic: true, label: "Floor" });
  let Ceiling = Matter.Bodies.rectangle(200, 0, 400, 20, { isStatic: true, label: "Ceiling" });
  
  // Start buddies far off screen
  let Obs1 = Matter.Bodies.rectangle(-1000, 100, 40, 40, { isStatic: true, label: "Obs" });
  let Obs2 = Matter.Bodies.rectangle(-1000, 200, 40, 40, { isStatic: true, label: "Obs" });
  let Obs3 = Matter.Bodies.circle(-1000, 300, 20, { isStatic: true, label: "Obs" });
  let Obs4 = Matter.Bodies.circle(-1000, 400, 20, { isStatic: true, label: "Obs" });
  let Obs5 = Matter.Bodies.rectangle(-1000, 500, 60, 20, { isStatic: true, label: "Obs" });

  Matter.World.add(world, [Player, Floor, Ceiling, Obs1, Obs2, Obs3, Obs4, Obs5]);
